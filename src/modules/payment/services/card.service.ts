...
export default class CardService {
  ...
  async authorizeAddCard(
    args: AuthorizeCardInput,
  ): Promise<AuthorizeCardOutput> {
    const { user_id } = args;
  ...
  async saveCard(args: SaveCardInput): Promise<SaveCardOutput> {
    ...
    const user =
      await this.userRepository.fetchOneByCustomerId(stripe_customer_id);
    ...
    const updateAccount = await this.userRepository.updateWithCustomerId(
      stripe_customer_id,
      {
        stripe_card_id,
