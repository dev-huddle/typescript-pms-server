...
export default class FileManagerService {
  ...
  async getAllObjects(args: GetAllObjectsInput): Promise<GetAllObjectsOutput> {
    ...
    const check_user_id =
      await this.userRepository.fetchOneByCognitoId(user_id);

    const totalObject = await this.fileManagerRepository.totalObjectByUser(
      check_user_id?._id!,
      folder,
    );
    ...
    const response = await this.fileManagerRepository.fetchAllByUserId(
      check_user_id?._id!,
      skip,
      limitNumber,
      folder,
  ...
  async deleteObject(args: DeleteObjectInput): Promise<DeleteObjectOutput> {
    ...
    const checkObject =
      await this.fileManagerRepository.fetchOneById(object_id);
  ...
  async deleteManyObjects(
    args: DeleteManyObjectInput,
  ): Promise<DeleteManyObjectOutput> {
